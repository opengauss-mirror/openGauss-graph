...
ExecNestLoopVLE(NestLoopVLEState *node)
{
    ...
    if (node->nls.js.ps.ps_TupFromTlist) {
        ...
        TupleTableSlot* result = ExecProject(node->nls.js.ps.ps_ProjInfo, &is_done);
	...
	for (;;)
	{
		...
		if (node->nls.nl_NeedNewOuter)
		{
			...
			if (TupIsNull(outerTupleSlot))
			{
				...
				ExecEarlyFree(innerPlan);
                ExecEarlyFree(outerPlan);
         ...
         */
        bool orig_value = innerPlan->state->es_skip_early_free;
        if (!IsA(innerPlan, MaterialState))
            innerPlan->state->es_skip_early_free = true;

		innerTupleSlot = ExecProcNode(innerPlan);

		innerPlan->state->es_skip_early_free = orig_value;
							...
							innerTupleSlot->tts_values[INNER_EID_VARNO]))
		{
			if (ExecQual(otherqual, econtext, false))
			{
				...
				if (canFollowEdge(node))
				{
					...
					ExprDoneCond is_done;
					if (needResult(node))
						result = ExecProject(node->nls.js.ps.ps_ProjInfo,&is_done);
					else
						result = NULL;
...
ExecInitNestLoopVLE(NestLoopVLE *node, EState *estate, int eflags)
{
	 ...
	 */
	nlvstate->nls.js.ps.targetlist = (List*)ExecInitExpr((Expr*)node->nl.join.plan.targetlist, (PlanState*)nlvstate);
	nlvstate->nls.js.ps.qual = (List*)ExecInitExpr((Expr*)node->nl.join.plan.qual, (PlanState*)nlvstate);
	...
	// ExecInitResultTupleSlotTL(&nlvstate->nls.js.ps);
	ExecAssignResultTypeFromTL(&nlvstate->nls.js.ps, TAM_HEAP);
	
	ExecAssignProjectionInfo(&nlvstate->nls.js.ps, NULL);

	nlvstate->curhops = getInitialCurhops(node);

	innerTupleDesc = innerPlanState(nlvstate)->ps_ResultTupleSlot->tts_tupleDescriptor;
...
storeOuterVar(NestLoopVLEState *node, TupleTableSlot *slot)
{
	...
	{
		ctx = (NestLoopVLEContext *)palloc(sizeof(*ctx));
		ctx_node = &ctx->list;
...
adjustResult(NestLoopVLEState *node, TupleTableSlot *slot)
{
	...
	slot->tts_values[OUTER_EIDS_VARNO] = makeArrayResultForVLE(node->eids, tupmctx);
	slot->tts_isnull[OUTER_EIDS_VARNO] = false;
	if (node->edges != NULL)
	{
		slot->tts_values[OUTER_EDGES_VARNO] = makeArrayResultForVLE(node->edges,
															  tupmctx);
		slot->tts_isnull[OUTER_EDGES_VARNO] = false;
	}
	if (node->vertices != NULL)
	{
		slot->tts_values[OUTER_VERTICES_VARNO] = makeArrayResultForVLE(node->vertices,
																 tupmctx);
