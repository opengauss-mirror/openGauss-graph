...
CreateGraphCommand(CreateGraphStmt *stmt, const char *queryString)
{
	...
	foreach(parsetree_item, parsetree_list)
	{
		...
		ProcessUtility(stmt,
					   queryString,
					   NULL,    // according to gs/schemacmds.cpp
					   NULL,
					   None_Receiver,
					   NULL,
					   NULL);
...
{
	Relation	gs_graph_desc;
	HeapTuple	tup;
	Form_gs_graph graphtup;
	NameData	graphname;

	gs_graph_desc = heap_open(GraphRelationId, RowExclusiveLock);

	tup = SearchSysCache1(GRAPHOID, ObjectIdGetDatum(graphid));
	if (!HeapTupleIsValid(tup))
		elog(ERROR, "cache lookup failed for graph %u", graphid);

	graphtup = (Form_gs_graph) GETSTRUCT(tup);
	namestrcpy(&graphname, (const char*)&graphtup->graphname.data);

	simple_heap_delete(gs_graph_desc, &tup->t_self);

	ReleaseSysCache(tup);

	heap_close(gs_graph_desc, RowExclusiveLock);
...
RenameGraph(const char *oldname, const char *newname)
{
	...
	/* rename */
	namestrcpy(&(((Form_gs_graph) GETSTRUCT(tup))->graphname), newname);
				   ...
				   ParamListInfo params)
{
	...
	foreach(l, stmts)
	{
		...
		else if (IsA(stmt, AlterSeqStmt))
		{
			/*
			 * ProcessUtility要求is_top_level设置为true才能执行alterseqstmt，
			 * 正确流程是在shell输入alterseq语句，直接在这里手动设置为is_top_level=true可能会导致的bug
			 */
			ProcessUtility(stmt, queryString, params, true, None_Receiver, NULL, NULL);
		} 
		else
		{
			/*
			...
			ProcessUtility(stmt, queryString, 
						   params, NULL, None_Receiver, NULL, NULL);
		}
...
DefineLabel(CreateStmt *stmt, char labkind)
{
	...
	relId = DefineRelation(stmt, RELKIND_RELATION, InvalidOid, NULL);
	ObjectAddressSet(reladdr, RelationRelationId, relId);
...
SetMaxStatisticsTarget(Oid laboid)
{
	...
	tuple = SearchSysCacheCopyAttName(laboid, GS_START_ID);
	...
	tuple = SearchSysCacheCopyAttName(laboid, GS_END_ID);
...
RenameLabel(RenameStmt *stmt)
{
	...
	/* rename */
	namestrcpy(&(((Form_gs_label) GETSTRUCT(tup))->labname), stmt->newname);
	CatalogTupleUpdate(rel, &tup->t_self, tup);
...
CheckLabelType(ObjectType type, Oid laboid, const char *command)
{
	...
	if (namestrcmp(&(labtup->labname), GS_VERTEX) == 0)
		ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
				 errmsg("%s base vertex label is prohibited", command)));
	if (namestrcmp(&(labtup->labname), GS_EDGE) == 0)
		ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
						...
						int stmt_len,ParamListInfo params)
{
	...
	ProcessUtility((Node*)wrapper, queryString,
				   params, NULL, None_Receiver, NULL, NULL);
...
DisableIndexCommand(DisableIndexStmt *disableStmt)
{
	...
	relid = RangeVarGetRelidExtended(relation, ShareLock, false, false, false, false, 
...
deleteRelatedEdges(const char *vlab)
{
	...
	gsedge = get_laboid_relid(get_labname_laboid(GS_EDGE, graphoid));
	edges = list_make1_oid(gsedge);
	edges = list_concat(edges, find_all_inheritors(gsedge, NoLock, NULL));
