...
transformShortestPath(ParseState *pstate, CypherPath *cpath)
{
	...
	checkRelFormat(pstate, (CypherRel *)lsecond(cpath->chain), false);
	checkNodeForRef(pstate, (CypherNode *)llast(cpath->chain));
...
transformShortestPathInMatch(ParseState *parentParseState, CypherPath *cpath)
{
	...
	checkNodeReferable(pstate, (CypherNode *)linitial(cpath->chain));
	checkRelFormat(pstate, (CypherRel *)lsecond(cpath->chain), true);
	checkNodeReferable(pstate, (CypherNode *)llast(cpath->chain));
...
makeShortestPathQuery(ParseState *pstate, CypherPath *cpath, bool isexpr)
{
	...
	if ( pathname != NULL || isexpr)
	{
		...
		expr = transformExpr(pstate, path);
	...
	{
		edges = makeEdgesSubLink(cpath, false);
		expr = transformExpr(pstate, edges);
...
makeShortestPathFrom(ParseState *parentParseState, CypherPath *cpath)
{
	...
	vertex = (CypherNode*)linitial(cpath->chain);
	...
	vertex = (CypherNode*)llast(cpath->chain);
	param = makeColumnRef1(getCypherName(vertex->variable));
	vertex_id = makeVertexIdExpr(param);
...
makeShortestpathEdgeUnionLeft(char *elabel_name, CypherRel *crel)
{
	...
	r->inhOpt = (crel->only) ? INH_NO : INH_YES;
	...
	lsel->targetList = lappend(lsel->targetList,
							   makeSimpleResTarget(GS_START_ID, NULL));
	lsel->targetList = lappend(lsel->targetList,
							   makeSimpleResTarget(GS_END_ID, NULL));
	lsel->targetList = lappend(lsel->targetList,
	...
	sel->targetList = list_make4(makeSimpleResTarget(GS_START_ID, SP_ALIASNAME_START_LEFT),
								 makeSimpleResTarget(GS_END_ID, SP_ALIASNAME_END_LEFT),
...
makeShortestpathEdgeUnionRight(char *elabel_name, CypherRel *crel)
{
	...
	sel->targetList = list_make4(makeSimpleResTarget(GS_START_ID, SP_ALIASNAME_START_RIGHT),
								 makeSimpleResTarget(GS_END_ID, SP_ALIASNAME_END_RIGHT),
...
makeVerticesSubLink(void)
{
	...
	id = makeColumnRef1(GS_ELEM_LOCAL_ID);

	selsub = makeNode(SelectStmt);

	vertex = makeRowExpr(list_make3(id,
									makeColumnRef1(GS_ELEM_PROP_MAP),
									makeColumnRef1("ctid")),
						 "vertex");
	selsub->targetList = list_make1(makeResTarget(vertex, NULL));

	gs_vertex = makeRangeVar(get_graph_path(true), GS_VERTEX, -1);
	gs_vertex->inhOpt = INH_YES;
	selsub->fromClause = list_make1(gs_vertex);
...
makeEdgesSubLink(CypherPath *cpath, bool is_dijkstra)
{
	...
	edge = makeRowExpr(list_make5(id,
								  makeColumnRef1(GS_START_ID),
								  makeColumnRef1(GS_END_ID),
								  makeColumnRef1(GS_ELEM_PROP_MAP),
								  makeColumnRef1("ctid")),
					   "edge");
	...
	if (is_dijkstra)
	{
		qual = makeSimpleA_Expr(AEXPR_OP, "=", (Node*)copyObject(id),
...
{
	return (Node *) makeFuncCall(list_make1(makeString(GS_ELEM_ID)),
								 list_make1(vertex),
								 -1);
}
...
makeAArrayExpr(List *elements, char *typeName)
{
	...
	cast->arg = (Node *) arr;
	cast->typname = makeTypeName(typeName);
	cast->location = -1;
...
transformDijkstra(ParseState *pstate, CypherPath *cpath)
{
	...
	checkNodeForRefForDijkstra(pstate, (CypherNode*)linitial(cpath->chain));
	checkRelFormatForDijkstra(pstate, (CypherRel*)lsecond(cpath->chain));
	checkNodeForRefForDijkstra(pstate, (CypherNode*)llast(cpath->chain));
...
transformDijkstraInMatch(ParseState *parentParseState, CypherPath *cpath)
{
	...
	checkNodeReferableForDijkstra(pstate, (CypherNode*)linitial(cpath->chain));
	checkRelFormatForDijkstra(pstate, (CypherRel*)lsecond(cpath->chain));
	checkNodeReferableForDijkstra(pstate, (CypherNode*)llast(cpath->chain));
...
makeDijkstraFrom(ParseState *parentParseState, CypherPath *cpath)
{
	...
	target = ParseFuncOrColumn(pstate, fc->funcname, NIL, (FuncCall*)pstate->p_last_srf, -1);
	te = makeTargetEntry((Expr *) target,
						 (AttrNumber) pstate->p_next_resno++,
						 "vids", false);
	qry->targetList = list_make1(te);
	...
	crel = (CypherRel*)lsecond(cpath->chain);
	if (crel->direction == CYPHER_REL_DIR_LEFT)
		target = transformExpr(pstate, makeColumnRef1(GS_START_ID));
	else
		target = transformExpr(pstate, makeColumnRef1(GS_END_ID));
	qry->dijkstraEndId = target;

	/* edge ID */
	target = transformExpr(pstate, makeColumnRef1(GS_ELEM_ID));
	...
	/* WHERE */
	if (crel->direction == CYPHER_REL_DIR_LEFT)
		start = makeColumnRef1(GS_END_ID);
	else
		start = makeColumnRef1(GS_START_ID);
...
makeDijkstraEdgeUnion(char *elabel_name, char *row_name)
{
	...
	lsel = makeNode(SelectStmt);
	lsel->targetList = lappend(lsel->targetList,
							   makeSimpleResTarget(GS_START_ID, "_start"));
	lsel->targetList = lappend(lsel->targetList,
							   makeSimpleResTarget(GS_END_ID, "_end"));
	lsel->targetList = lappend(lsel->targetList,
	...
	lsel->targetList = lappend(lsel->targetList,
							   makeSimpleResTarget(GS_END_ID, NULL));
	lsel->targetList = lappend(lsel->targetList,
							   makeSimpleResTarget(GS_ELEM_LOCAL_ID, NULL));
	if (row_name != NULL)
		lsel->targetList = lappend(lsel->targetList,
								   makeSimpleResTarget(GS_ELEM_PROP_MAP, NULL));
	...
	sel->targetList = list_make4(makeSimpleResTarget(GS_START_ID, NULL),
								 makeSimpleResTarget(GS_END_ID, NULL),
								 makeSimpleResTarget(GS_ELEM_LOCAL_ID, NULL),
								 makeSimpleResTarget("ctid", NULL));
	if (row_name != NULL)
	{
		row = makeRowExpr(list_make5(makeColumnRef1(GS_ELEM_LOCAL_ID),
