 ...
 *
 * IDENTIFICATION
 *	  src/backend/parser/parse_graph.cpp
 */
#ifdef GS_GRAPH
#include "postgres.h"
...
static Node *makeSelectEdgesVertices(Node *vertices,
									 CypherDeleteClause *cypher_delete,
									 char **edges_resname);
...
#ifdef GS_GRAPH
std::string addQuotationMarks(std::string ori_string);
#endif
...
transformCypherSubPattern(ParseState *pstate, CypherSubPattern *subpat)
{
	...
	if (subpat->kind == CSP_FINDPATH)
	{
		...
		cp = (CypherPath*)linitial(subpat->pattern);
	...
	if (subpat->kind == CSP_SIZE)
	{
		...
		te = transformTargetEntry(pstate, (Node *) count, NULL, NULL, false);
...
transformCypherCreateClause(ParseState *pstate, CypherClause *clause)
{
	...
	cpath = (CypherPath*)llast(detail->pattern);
...
Query *
transformCypherCreateClauseSimple(ParseState *pstate, CypherClause *clause){
	CypherCreateClause *detail = (CypherCreateClause*) clause->detail;
	CypherPath *path = (CypherPath*) llast(detail->pattern);
	List* chain = path->chain;
	ListCell* cnode;
	TargetEntry* entry;
	A_Const* n;
	FuncCall* cypher_create;
	List* args;
	Query* qry;

	Assert(list_length(detail->pattern) == 1);
	std::string vtags,etags,vprops,eprops;
	// get information from the chain
	int nodeflag = 0;
	foreach(cnode, chain){
		List* props;
		ListCell* pnode;
		...
		if(nodeflag%2==0)
		{
			CypherNode* cn = (CypherNode*) cnode->data.ptr_value;
			vtags += addQuotationMarks(((CypherName*)cn->label)->name);
			vtags += ",";
			props = ((CypherMapExpr*)cn->prop_map)->keyvals;
		}
		// edge
		else 
		{
			CypherRel* cr = (CypherRel*) cnode->data.ptr_value;;
			etags += addQuotationMarks(((CypherName*)cr->types->head->data.ptr_value)->name);
			etags += ",";
			props = ((CypherMapExpr*)cr->prop_map)->keyvals;
		}
		// properties
		foreach(pnode, props){
			//property name
			...
			{
				prop = "{";
				prop += addQuotationMarks(((Value*)pnode->data.ptr_value)->val.str);
			}
			//property value
			else
			{
				prop = ":";
				prop += addQuotationMarks(((A_Const*)pnode->data.ptr_value)->val.val.str);
				prop += "},";
			}
			if(nodeflag%2==0)
				vprops+=prop;
	...
	// assemble the string
	std::string gp =get_graph_path(true);
	std::string finalstr = "{\"graph_path\":\""+gp+"\",";
	vtags = "\"vtags\":[" + vtags.substr(0,vtags.size()-1) + "],";
	finalstr += vtags;
	etags = "\"etags\":[" + etags.substr(0,etags.size()-1) + "],";
	finalstr += etags;
	vprops = "\"vprops\":[" + vprops.substr(0,vprops.size()-1) + "],";
	finalstr += vprops;
	eprops = "\"eprops\":[" + eprops.substr(0,eprops.size()-1) + "]}";
	finalstr += eprops;

	n = makeNode(A_Const);
	n->val.type = T_String;
	n->val.val.str = (char*)palloc(finalstr.size()+1);
	strncpy(n->val.val.str, finalstr.c_str(),finalstr.size());
	n->val.val.str[finalstr.size()] = '\0';
	args = list_make1(n);

	cypher_create = makeFuncCall(list_make1(makeString("cypher_create")), args, -1);
	entry = transformTargetEntry(pstate, (Node*)cypher_create, NULL, NULL, false);
	
	qry = makeNode(Query);
	qry->commandType=CMD_SELECT;
	qry->targetList = list_make1(entry);
...
transformCypherDeleteClause(ParseState *pstate, CypherClause *clause)
{
	...
	if (detail->detach && pstate->p_delete_edges_resname)
	{
		...
		te = (TargetEntry*)llast(qry->targetList);
		edges = (Node*)llast(detail->exprs);
	...
	{
		GraphDelElem *gde = (GraphDelElem*)lfirst(le);
...
transformCypherSetClause(ParseState *pstate, CypherClause *clause)
{
	...
	{
		GraphSetProp *gsp = (GraphSetProp*)lfirst(le);
...
transformCypherLoadClause(ParseState *pstate, CypherClause *clause)
{
	...
	rte = addRangeTableEntry(pstate, rv, rv->alias, rv->inhOpt, true);
...
checkNameInItems(ParseState *pstate, List *items, List *targetList)
{
	...
	{
		ResTarget *res = (ResTarget*)lfirst(li);
		TargetEntry *te = (TargetEntry*)lfirst(lt);
...
transformMatchOptional(ParseState *pstate, CypherClause *clause)
{
	...
	r_rte = transformClauseImpl(pstate, (Node *) clause, (TransformMethod)transformStmt,
								r_alias);
...
appendFindPathsResult(ParseState *pstate, List *fplist, List **targetList)
{
	...
	foreach(le, fplist)
	{
		CypherPath *p = (CypherPath*)lfirst(le);
		CypherRel  *crel;
		char	   *pathname;
		char	   *edgename;
		char	   *weightvar;
		...
		{
			crel = (CypherRel*)lsecond(p->chain);
...
collectNodeInfo(ParseState *pstate, List *pattern)
{
	...
	foreach(lp, pattern)
	{
		...
		foreach(le, p->chain)
		{
			CypherNode *cnode = (CypherNode*)lfirst(le);
...
findNodeInfo(ParseState *pstate, char *varname)
{
	...
	{
		NodeInfo *ni = (NodeInfo*)lfirst(le);
...
arePathsConnected(CypherPath *path1, CypherPath *path2)
{
	...
	{
		CypherNode *cnode1 = (CypherNode*)lfirst(le1);
		char	   *varname1;
		ListCell   *le2;
		...
		{
			CypherNode *cnode2 = (CypherNode* )lfirst(le2);
			char	   *varname2;
...
transformComponents(ParseState *pstate, List *components, List **targetList)
{
	...
	{
		ElemQualOnly *eqo = (ElemQualOnly*)lfirst(leqo);
				   ...
				   List **targetList, List **eqoList)
{
	 ...
	 */
	if (strcmp(labname, GS_VERTEX) != 0 || prop_constr || force)
	{
		RangeVar   *r;
		Alias	   *alias;
		RangeTblEntry *rte;
		...
		if (varname != NULL || prop_constr)
		{
			...
			if (resjunk)
			{
				...
				eqo = (ElemQualOnly*)palloc(sizeof(*eqo));
				eqo->te = te;
...
genEdgeUnion(char *edge_label, bool only, int location)
{
	...
	id = makeSimpleResTarget(GS_ELEM_LOCAL_ID, NULL);
	start = makeSimpleResTarget(GS_START_ID, NULL);
	end = makeSimpleResTarget(GS_END_ID, NULL);
	prop_map = makeSimpleResTarget(GS_ELEM_PROP_MAP, NULL);
	tid = makeSimpleResTarget("ctid", NULL);

	r = makeRangeVar(get_graph_path(true), edge_label, location);
	r->inhOpt = InhOption(!only);
	...
	lsel->targetList = lappend(lsel->targetList,
							   makeSimpleResTarget(GS_START_ID,
												   EDGE_UNION_START_ID));
	lsel->targetList = lappend(lsel->targetList,
							   makeSimpleResTarget(GS_END_ID,
												   EDGE_UNION_END_ID));
					...
					CypherRel *prev_crel, RangeTblEntry *prev_edge)
{
	...
	else
	{
		...
		vid = (Node *) makeFuncCall(list_make1(makeString(GS_ELEM_ID)),
									list_make1(cref), -1);
...
genVLELeftChild(ParseState *pstate, CypherRel *crel, bool out, bool pathout)
{
	...
	if (isZeroLengthVLE(crel))
	{
		...
		if (out)
		{
			...
			edges->typname = makeTypeName("_edge");
			edges->location = -1;
		...
		if (pathout)
		{
			...
			vertices->typname = makeTypeName("_vertex");
			vertices->location = -1;
	...
	else
	{
		...
		if (out)
		{
			...
			cast->typname = makeTypeName("_edge");
			cast->location = -1;
		...
		if (pathout)
		{
			...
			cast->typname = makeTypeName("_vertex");
			cast->location = -1;
...
genVLERightChild(ParseState *pstate, CypherRel *crel, bool out, bool pathout)
{
	...
	if (pathout)
	{
		...
		vtx = makeRangeVar(get_graph_path(true), GS_VERTEX, -1);
		...
		vtxid->fields = genQualifiedName(VLE_VERTEX_ALIAS, GS_ELEM_ID);
		vtxid->location = -1;
...
genVertexSimple(char *aliasname)
{
	...
	id = makeColumnRef(genQualifiedName(aliasname, GS_ELEM_LOCAL_ID));
	prop_map = makeColumnRef(genQualifiedName(aliasname, GS_ELEM_PROP_MAP));
	...
	vertex->arg = (Node *) row;
	vertex->typname = makeTypeName("vertex");
	vertex->location = -1;
...
genEdgeSimple(char *aliasname)
{
	...
	id = makeColumnRef(genQualifiedName(aliasname, GS_ELEM_LOCAL_ID));
	start = makeColumnRef(genQualifiedName(aliasname, GS_START_ID));
	end = makeColumnRef(genQualifiedName(aliasname, GS_END_ID));
	prop_map = makeColumnRef(genQualifiedName(aliasname, GS_ELEM_PROP_MAP));
	tid = makeColumnRef(genQualifiedName(aliasname, "ctid"));
	...
	edge->arg = (Node *) row;
	edge->typname = makeTypeName("edge");
	edge->location = -1;
...
genVLEJoinExpr(CypherRel *crel, Node *larg, Node *rarg)
{
	...
	truecond->typname = makeTypeNameFromNameList(
...
genVLEQual(char *alias, Node *propMap)
{
	...
	prop->fields = genQualifiedName(alias, GS_ELEM_PROP_MAP);
	prop->location = -1;
			  ...
			  RangeTblEntry *edge, bool prev)
{
	...
	else
	{
		...
		id = getExprField(te->expr, GS_ELEM_ID);
	}
...
adjustElemQuals(List *elem_quals, RangeTblEntry *rte, int rtindex)
{
	...
	{
		ElemQual *eq = (ElemQual*)lfirst(le);
...
transformElemQuals(ParseState *pstate, Node *qual)
{
	...
	foreach(le, pstate->p_elem_quals)
	{
		...
		prop_map = getExprField((Expr *) var, GS_ELEM_PROP_MAP);
...
transform_prop_constr_worker(Node *node, prop_constr_context *ctx)
{
	...
	while (le != NULL)
	{
		...
		else
		{
			...
			a->path = (List* )copyObject(ctx->pathelems);
			lval = (Node *) a;
			...
			case RTE_SUBQUERY:
				{
					...
					if (IsA(te->expr, Var))
						...
						var = (Var *)linitial(((RowExpr *) te->expr)->args);
					else
						return InvalidOid;
			...
			case RTE_JOIN:
				{
					...
					expr = (Expr*)list_nth(rte->joinaliasvars, varattno - 1);
					if (!IsA(expr, Var))
						return InvalidOid;
	...
	foreach(li, indexoidlist)
	{
		...
		if (index->indcheckxmin &&
			!TransactionIdPrecedes(
					HeapTupleHeaderGetXmin1(indexRel->rd_indextuple->t_data),
					TransactionXmin))
		{
			index_close(indexRel, NoLock);
			continue;
		...
		attnum = attnameAttNum(rel, GS_ELEM_PROP_MAP, false);
				 ...
				 int sublevels_up)
{
	...
	{
		FutureVertex *fv = (FutureVertex* )lfirst(le);
...
resolve_future_vertex_mutator(Node *node, resolve_future_vertex_context *ctx)
{
	...
	if (IsA(node, Aggref))
	{
		...
		if (agglevelsup == ctx->sublevels_up)
		{
			...
			foreach(la, agg->args)
			{
				TargetEntry *arg = (TargetEntry* )lfirst(la);
	...
	if (IsA(node, Query))
	{
		...
		newnode = query_tree_mutator((Query *) node,
									 (Node* (*)(Node*, void*))resolve_future_vertex_mutator, ctx, 0);
		ctx->sublevels_up--;

		return (Node *) newnode;
	}

	return expression_tree_mutator(node, (Node* (*)(Node*, void*))resolve_future_vertex_mutator, ctx);
}
...
resolveFutureVertex(ParseState *pstate, FutureVertex *fv, bool ignore_nullable)
{
	...
	fv_id = getExprField((Expr *) fv_var, GS_ELEM_ID);
	...
	sel_id = makeFuncCall(list_make1(makeString(GS_ELEM_ID)), NIL, -1);
	id = ParseFuncOrColumn(pstate, sel_id->funcname, list_make1(vertex),
						   (FuncCall *)pstate->p_last_srf, sel_id->location, -1);
	...
	else
	{
		...
		l_jt = (Node* )llast(pstate->p_joinlist);
...
transformCreateNode(ParseState *pstate, CypherNode *cnode, List **targetList)
{
	...
	if (create)
	{
		...
		te = makeTargetEntry((Expr *) vertex,
							 (AttrNumber) pstate->p_next_resno++,
							 (varname == NULL ? const_cast<char*>("?column?") : varname),
							 false);
...
makeNewVertex(ParseState *pstate, Relation relation, Node *prop_map)
{
	...
	id_attnum = attnameAttNum(relation, GS_ELEM_LOCAL_ID, false);
	Assert(id_attnum == 1);
	id = build_column_default(relation, id_attnum);

	prop_map_attnum = attnameAttNum(relation, GS_ELEM_PROP_MAP, false);
					 ...
					 CSetKind kind, List *items)
{
	...
	foreach(li, items)
	{
		CypherSetProp *sp = (CypherSetProp* )lfirst(li);
				 ...
				 bool is_remove, CSetKind kind)
{
	...
	prop_map = ParseFuncOrColumn(pstate,
								 list_make1(makeString(GS_ELEM_PROP_MAP)),
								 list_make1(elem), (FuncCall*)pstate->p_last_srf,
								 NULL, -1);
	...
	if (path == NULL)	/* LHS is the property map itself */
	{
		...
		if (sp->add)
		{
			...
			prop_map = ParseFuncOrColumn(pstate, concat->funcname,
										 ...
										 (FuncCall*)pstate->p_last_srf, concat->location, -1);
		}
		else
		{
	...
	else				/* LHS is a property in the property map */
	{
		...
		cypher_delete = makeFuncCall(list_make1(makeString("jsonb_delete_path")),
							  NIL, -1);
		del_prop = ParseFuncOrColumn(pstate, cypher_delete->funcname,
									 list_make2(prop_map, path),
									 (FuncCall *)pstate->p_last_srf, cypher_delete->location, -1);
		...
		else							/* SET a.b.c = expr */
		{
			...
			set_prop = ParseFuncOrColumn(pstate, set->funcname,
										 ...
										 (FuncCall*)pstate->p_last_srf, set->location, -1);
...
transformMergeMatchJoin(ParseState *pstate, CypherClause *clause)
{
	...
	r_rte = transformClauseImpl(pstate, makeMatchForMerge(detail->pattern),
								(TransformMethod)transformStmt, r_alias);
					 ...
					 List *resultList)
{
	 ...
	 */
	prevtlist = (List* )copyObject(prevrte->subquery->targetList);

	path = (CypherPath*)linitial(pattern);
				  ...
				  List *resultList)
{
	...
	relation = openTargetLabel(pstate, getCypherName((Node* )linitial(crel->types)));
...
transformDeleteJoinRTE(ParseState *pstate, CypherClause *clause)
{
	...
	sel_gs_edge = makeSelectEdgesVertices(vertices, detail, &edges_resname);
	...
	r_qry = parse_sub_analyze(sel_gs_edge, pstate, NULL,
 ...
 *     FROM gs_edge AS e, unnest(vertices) AS v
 ...
 */
static Node *
makeSelectEdgesVertices(Node *vertices, CypherDeleteClause *cypher_delete, char **edges_resname)
{
	List	   *targetlist = NIL;
	RangeVar   *gs_edge;
	...
	if (cypher_delete->detach)
	{
		...
		cypher_delete->exprs = lappend(cypher_delete->exprs, edges_col);
	...
	else
	{
		...
		nulledge->typname = makeTypeName("edge");
		nulledge->location = -1;

		targetlist = list_make1(makeResTarget((Node *) nulledge, NULL));
	}

	gs_edge = makeRangeVar(get_graph_path(true), GS_EDGE, -1);
	gs_edge->inhOpt = INH_YES;
	gs_edge->alias = makeAliasNoDup(DELETE_EDGE_ALIAS, NIL);

	unnest = makeUnnestVertices(vertices);

	sel = makeNode(SelectStmt);
	sel->targetList = targetlist;
	sel->fromClause = list_make2(gs_edge, unnest);
...
makeEdgesForDetach(void)
{
	...
	id = makeColumnRef(genQualifiedName(DELETE_EDGE_ALIAS, GS_ELEM_ID));
	start = makeColumnRef(genQualifiedName(DELETE_EDGE_ALIAS, GS_START_ID));
	end = makeColumnRef(genQualifiedName(DELETE_EDGE_ALIAS, GS_END_ID));
...
makeEdgesVertexQual(void)
{
	...
	vid = makeColumnRef(genQualifiedName(DELETE_VERTEX_ALIAS, GS_ELEM_ID));
...
extractVerticesExpr(ParseState *pstate, List *exprlist, ParseExprKind exprKind)
{
	...
	foreach(le, exprlist)
	{
		...
		switch (exprType(elem))
		{
			...
			case GRAPHPATHOID:
				elem = getExprField((Expr *) elem, GS_PATH_VERTICES);
				/* no break */
			case VERTEXOID:
				{
					GraphDelElem *gde = makeNode(GraphDelElem);
...
extractEdgesExpr(ParseState *pstate, List *exprlist, ParseExprKind exprKind)
{
	...
	foreach(le, exprlist)
	{
		...
		switch (exprType(elem))
		{
			...
			case GRAPHPATHOID:
				elem = getExprField((Expr *) elem, GS_PATH_EDGES);
				/* no break */
			case EDGEOID:
				{
					GraphDelElem *gde = makeNode(GraphDelElem);
...
find_target_label_walker(Node *node, find_target_label_context *ctx)
{
	...
	if (IsA(node, Var))
	{
		...
		else if (rte->rtekind == RTE_JOIN)
		{
			...
			joinvar = (Node* )list_nth(rte->joinaliasvars, var->varattno - 1);
	...
	if (IsA(node, RowExpr) && qry->graph.writeOp == GWROP_CREATE)
	{
		...
		gpath = (GraphPath* )linitial(qry->graph.pattern);
	...
	if (IsA(node, FieldSelect))
	{
		...
		if (exprType((Node *) fs->arg) == GRAPHPATHOID)
		{
			...
			{
				ctx->relid = get_laboid_relid(get_labname_laboid(GS_VERTEX,
																 graph_oid));
			...
			{
				ctx->relid = get_laboid_relid(get_labname_laboid(GS_EDGE,
																 graph_oid));
	...
	if (expression_tree_walker(node, (bool (*)())find_target_label_walker, ctx))
		return true;
...
static bool
labelExist(ParseState *pstate, char *labname, int labloc, char labkind, bool throw_exist)
{
	Oid			graphid;
	HeapTuple	tuple;
	char	   *elemstr;
...
stripNullKeys(ParseState *pstate, Node *properties)
{
	...
	return ParseFuncOrColumn(pstate, strip->funcname, list_make1(properties),
							 (FuncCall *)pstate->p_last_srf, strip->location, -1);
...
assign_query_eager_walker(Node *node, Query *nxtQry)
{
	...
	if (IsA(node, Query))
	{
		...
		(void) range_table_walker(qry->rtable,
								  (bool (*)())assign_query_eager_walker,
								  (void *)nxtQry,
...
{
	return transformClauseBy(pstate, clause, (TransformMethod)transformStmt);
}
...
std::string addQuotationMarks(std::string ori_string){
	std::string result = "\"";
	if(ori_string.size()>=2&&ori_string[0]=='\''&&ori_string[ori_string.size()-1]=='\'')
		result += ori_string.substr(1,ori_string.size()-2);
	else
		result += ori_string;
	result += "\"";
	return result;
