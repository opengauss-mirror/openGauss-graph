...
static size_t format_and_insert_text(char *buf, size_t len, const char *fmt, va_list args);

/* GUC variable (allow/disallow null properties) */
bool		allow_null_properties = false;

// function cypher to jsonb is defined in src/common/backend/catalog/builtin_funcs.ini
#define F_CYPHER_TO_JSONB_OID 8115
...
transformCypherExprRecurse(ParseState *pstate, Node *expr)
{
	...
	{
		case T_ColumnRef:
			return transformColumnRef_cypher(pstate, (ColumnRef *) expr);
		case T_ParamRef:
		...
		case T_SubLink:
			{
				...
				return transformExpr(pstate, expr);
			}
 ...
 */
static Node *
transformColumnRef_cypher(ParseState *pstate, ColumnRef *cref)
{
	...
	if (nfields >= 3)
	{
		field3 = (Node*)lthird(cref->fields);
		nspid1 = LookupNamespaceNoError(strVal(field1));
	}
	if (nfields >= 4 &&
		strcmp(strVal(field1), get_database_name(u_sess->proc_cxt.MyDatabaseId)) == 0)
	{
		field4 = (Node*)lfourth(cref->fields);
	...
	while (pstate_up != NULL)
	{
		...
		foreach(lni, pstate_up->p_relnamespace)
		{
...
transformFields(ParseState *pstate, Node *basenode, List *fields, int location)
{
	...
	foreach(lf, fields)
	{
		...
		field = (Value*)lfirst(lf);

		res = ParseFuncOrColumn(pstate, list_make1(field), list_make1(res),
								(FuncCall *)pstate->p_last_srf, NULL, location);
		if (res == NULL)
		{
			ereport(ERROR,
					(errcode(ERRCODE_UNDEFINED_COLUMN),
	...
	switch (exprtype)
	{
		...
		case EDGEOID:
			return ParseFuncOrColumn(pstate,
									 ...
									 list_make1(expr), (FuncCall *)pstate->p_last_srf,
									 NULL, location);
		case JSONBOID:
			return expr;
...
transformTypeCast(ParseState *pstate, TypeCast *tc)
{
	...
	typenameTypeIdAndMod(pstate, tc->typname, &otyp, &otypmod);
...
transformFuncCall(ParseState *pstate, FuncCall *fn)
{
	...
	return ParseFuncOrColumn(pstate, fn->funcname, args, fn,
							 fn->location);
}
...
preprocess_func_args(ParseState *pstate, FuncCall *fn)
{
	...
	foreach(la, fn->args)
	{
		...
		if (isSubstr && nargs == 1)
		{
			...
			la->data.ptr_value = makeSimpleA_Expr(AEXPR_OP, "+", (Node*)lfirst(la),
												  (Node *) aconst, -1);
		}

		arg = transformCypherExprRecurse(pstate, (Node*)lfirst(la));
	...
	foreach(li, indir->indirection)
	{
		...
		if (IsA(i, String))
		{
			...
			res = ParseFuncOrColumn(pstate, list_make1(i), list_make1(res),
									(FuncCall*)last_srf, NULL, location);
			if (res == NULL)
			{
				ereport(ERROR,
						(errcode(ERRCODE_UNDEFINED_COLUMN),
...
transformAExprOp(ParseState *pstate, A_Expr *a)
{
	...
	if (a->kind == AEXPR_OP && list_length(a->name) == 1)
	{
			...
			strcmp(opname, "`^`") == 0)
		{
			...
			return (Node *) make_op(pstate, a->name, l, r,
									a->location);
		}
			...
			strcmp(opname, "^") == 0)
		{
							...
							true, a->location)))
			{
				...
				newopname = format_text("`%s`", opname);
...
transformAExprIn(ParseState *pstate, A_Expr *a)
{
	...
	foreach(l, rexprs)
	{
		...
		cmp = (Node *) make_op(pstate, a->name, (Node*)copyObject(lexpr), rexpr,
							   a->location);
			...
			CoercionContext cctx, CoercionForm cform, int loc)
{
		...
		!IsPolymorphicType(otyp))
	{
		...
		if (otyp == JSONBOID)
		{
			return (Node *) makeFuncExpr(F_CYPHER_TO_JSONB_OID, JSONBOID,
...
coerce_unknown_const(ParseState *pstate, Node *expr, Oid ityp, Oid otyp)
{
	...
	else if (otyp == TEXTOID)
	{
		...
		t = cstring_to_text_with_len(jv->string.val, jv->string.len);
...
Datum stringToJsonbSimple(char *s)
{
	StringInfoData si;
	const char *c;
	bool		escape = false;
...
coerce_all_to_jsonb(ParseState *pstate, Node *expr)
{
	...
	if (TypeCategory(getBaseType(type)) == TYPCATEGORY_STRING)
	{
		return ParseFuncOrColumn(pstate,
								 ...
								 list_make1(expr),
								 (FuncCall *)pstate->p_last_srf, NULL,
								 exprLocation(expr));
	}
						 ...
						 char **varname)
{
	...
	if (IsA(elem, Var))
	{
		...
		if (var->varlevelsup == 0)
		{
			...
			te = (TargetEntry*)list_nth(rte->subquery->targetList, var->varattno - 1);
...
transformCypherOrderBy(ParseState *pstate, List *sortitems, List **targetlist)
{
	...
	{
		SortBy	   *sortby = (SortBy*)lfirst(lsi);
		Node	   *expr;
		ListCell   *lt;
		TargetEntry *te = NULL;
		...
		if (te == NULL)
		{
			te = transformTargetEntry(pstate, sortby->node, expr, NULL, true);
