 ...
 *
 * IDENTIFICATION
 *	  src/backend/utils/adt/graph.cpp
 */
#ifdef GS_GRAPH
#include "postgres.h"
...
graphid_out(PG_FUNCTION_ARGS)
{
	...
	buf = (char*)palloc(GRAPHID_BUFLEN);
...
vertex_out(PG_FUNCTION_ARGS)
{
	...
	JsonbToCString(&si, VARDATA(prop_map), VARSIZE(prop_map));
...
/* note: It belongs to  array_expanded.cpp*/
/*
...
{
	const char *querystr =
			"SELECT (" GS_ELEM_LOCAL_ID ", " GS_ELEM_PROP_MAP ", NULL)::vertex "
			"FROM \"%s\"." GS_VERTEX " WHERE " GS_ELEM_LOCAL_ID " = $1";
	char		sqlcmd[256];
	...
	ret = SPI_execute_with_args(sqlcmd, 2, argTypes, values, NULL, true, 0, NULL);
...
cache_labels(FmgrInfo *flinfo, uint16 labid)
{
	...
	if (my_extra->label_labid != labid)
	{
		...
		foreach(li, ancestor_relids)
		{
			...
			if (strcmp(ancestor_labname, "gs_vertex") != 0)
			{
				JsonbValue	jv;
...
gin_extract_value_graphid(PG_FUNCTION_ARGS)
{
	...
	Datum	   *entries = (Datum*)palloc(sizeof(*entries) * _nentries);
...
gin_extract_query_graphid(PG_FUNCTION_ARGS)
{
	...
	*nentries = _nentries;
	*partial_matches = (bool*)palloc(sizeof(**partial_matches) * _nentries);
	entries = (Datum*)palloc(sizeof(*entries) * _nentries);
	...
	switch (strategy)
	{
		...
		case BTLessEqualStrategyNumber:
			 ...
			 */
			{
				...
				*extra_data = (char**)palloc(sizeof(**extra_data) * _nentries);
