...
{
	GsStat_GraphMeta	*meta_elem;
	HASH_SEQ_STATUS		seq;
	HeapTuple			tup;

	hash_seq_init(&seq, htab);

	while ((meta_elem = (GsStat_GraphMeta*)hash_seq_search(&seq)) != NULL)
	{
		Datum	values[Natts_gs_graphmeta];
		bool	isnull[Natts_gs_graphmeta];
		int		i;

		for (i = 0; i < Natts_gs_graphmeta; i++)
		{
			values[i] = (Datum) NULL;
			isnull[i] = false;
		}

		values[Anum_gs_graphmeta_graph -1] = ObjectIdGetDatum(meta_elem->key.graph);
		values[Anum_gs_graphmeta_edge - 1] = Int16GetDatum(meta_elem->key.edge);
		values[Anum_gs_graphmeta_start - 1] = Int16GetDatum(meta_elem->key.start);
		values[Anum_gs_graphmeta_end - 1] = Int16GetDatum(meta_elem->key.end);
		values[Anum_gs_graphmeta_edgecount - 1] = Int64GetDatum(meta_elem->edges_inserted);
...
scan_label(Oid relid, Oid graphid)
{
	...
	scan = (HeapScanDesc)heap_beginscan(rel, snapshot, 0, NULL);

	memset(&key, 0, sizeof(key));

	while ((tup = heap_getnext((TableScanDesc)scan, ForwardScanDirection)) != NULL)
	{
		Datum	dat;
		Labid	edge;
		Labid	start;
		...
		key.start = start;
		key.end = end;
		meta_elem= (GsStat_GraphMeta *) hash_search(htab,
													(void *) &key,
													HASH_ENTER, &found);
		if (found)
...
regather_graphmeta(PG_FUNCTION_ARGS)
{
	...
	while ((tup = heap_getnext((TableScanDesc)scan, ForwardScanDirection)) != NULL)
	{
		...
		labkind = heap_getattr(tup, Anum_gs_label_labkind,
							   RelationGetDescr(rel), &isnull);
		...
		relid = heap_getattr(tup, Anum_gs_label_relid,
							 RelationGetDescr(rel), &isnull);

		graphid = heap_getattr(tup, Anum_gs_label_graphid,
							   RelationGetDescr(rel), &isnull);
