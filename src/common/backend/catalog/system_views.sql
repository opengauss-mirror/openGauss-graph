...
-- CREATE VIEW gs_property_indexes AS
--     SELECT
...
--         gs_get_propindexdef(I.oid) AS indexdef,
--         pg_get_userbyid(c.relowner) AS owner,
--         pg_size_pretty(pg_table_size(c.oid)) AS size,
--         obj_description(i.oid, 'pg_class') as description
--     FROM pg_index X JOIN pg_class C ON (C.oid = X.indrelid)
--         JOIN gs_label L ON (X.indrelid = L.relid)
--         JOIN pg_class I ON (I.oid = X.indexrelid)
--         LEFT JOIN pg_namespace N ON (N.oid = C.relnamespace)
--         LEFT JOIN pg_tablespace T ON (T.oid = I.reltablespace)
...
-- CREATE VIEW gs_graphmeta_view AS
...
CREATE OR REPLACE FUNCTION cypher_create(TEXT) RETURNS TEXT AS $$
DECLARE
    params json := $1::json;
    graph_path TEXT := params->>'graph_path';
    vtags TEXT[] := ARRAY(select json_array_elements(params->'vtags'));
    etags TEXT[] := ARRAY(select json_array_elements(params->'etags'));
    vprops TEXT[] := ARRAY(select json_array_elements(params->'vprops'));
    eprops TEXT[] := ARRAY(select json_array_elements(params->'eprops'));
    graph_oid integer;
    DECLARE labid integer;
    vtag_oids integer[];
    vid graphid[];
    etag_oids integer[];
    eid graphid[];
    ...
    tag TEXT;
BEGIN
    SELECT nspid FROM gs_graph WHERE graphname=graph_path into graph_oid;

    -- The graphid in the gs_label is one larger than the graphid in the gs_graph
    graph_oid = graph_oid + 1;

    -- set graphid for each vertex
    FOR i in 1..array_length(vtags, 1) LOOP
        vtags[i] := replace(vtags[i], '"', '');
        SELECT labid FROM gs_label WHERE graphid=graph_oid and labname=vtags[i] into vtag_oids[i];
        select pg_get_serial_sequence( graph_path || '.' || vtags[i], 'id') into seq_name;
        select graphid(vtag_oids[i] || '.' || nextval(seq_name)) into vid[i];
    END LOOP;
    
    -- set graphid for each edge 
    FOR i in 1..array_length(etags, 1) LOOP
        etags[i] := replace(etags[i], '"', '');
        SELECT labid FROM gs_label WHERE graphid=graph_oid and labname=etags[i] into etag_oids[i];
        select pg_get_serial_sequence( graph_path || '.' || etags[i], 'id') into seq_name;
        select graphid(etag_oids[i] || '.' || nextval(seq_name)) into eid[i];
    END LOOP; 

    -- insert vertex data into tables;
    -- It is worth noting that vertex data is automatically inserted into gs_vertex when processing the following insert statement
    FOR i in 1..array_length(vtags, 1) LOOP
        tablename = graph_path || '.' || vtags[i];
        EXECUTE format('insert into %s(id, properties) values ($1, $2)', tablename) USING vid[i], vprops[i]::jsonb;
    END LOOP;

    -- insert edge data into tables;
    -- It is worth noting that edge data is automatically inserted into gs_edge when processing the following insert statement
    FOR i in 1..array_length(etags, 1) LOOP
        tablename = graph_path || '.' || etags[i];
        EXECUTE format('insert into %s(id, "start", "end", properties) values ($1, $2, $3, $4)', tablename) USING eid[i], vid[i], vid[i+1], eprops[i]::jsonb;
    END LOOP;
